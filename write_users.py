...
<div id="modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);z-index:9999;justify-content:center;align-items:center;">
<div style="background:linear-gradient(135deg,#1a1a1a,#2a2a2a);border:2px solid #dc143c;border-radius:20px;padding:40px;max-width:550px;width:90%;position:relative;box-shadow:0 20px 60px rgba(220,20,60,0.5);max-height:90vh;overflow-y:auto;">
<button onclick="closeModal()" style="position:absolute;top:15px;right:20px;background:none;border:none;color:#dc143c;font-size:28px;cursor:pointer;">&times;</button>
...
async function loadMembers() {
 ...
 document.getElementById('members-list').innerHTML = '<p style="text-align:center;color:#aaa;padding:40px;"><i class="fas fa-spinner fa-spin" style="font-size:30px;color:#dc143c;"></i><br><br>Yukleniyor...</p>';
...
function renderMembers(members) {
 ...
 card.style.cssText = 'background:linear-gradient(135deg,#1a1a1a,#2a2a2a);border:1px solid rgba(220,20,60,0.3);border-radius:12px;padding:20px;cursor:pointer;text-align:center;transition:all 0.3s;';

...
<div class="cmd-grid">
 <div class="cmd-card" onclick="openCmd(\'бан\')"><div class="cmd-icon">🔨</div><h3><i class="fas fa-ban"></i> Бан</h3><p>Пользователя постоянный забанить</p></div>
 ...
 <div class="cmd-card" onclick="openCmd(\'роли\')"><div class="cmd-icon">🏷️</div><h3><i class="fas fa-user-tag"></i> Роли Ver/Al</h3><p>Роли add или cыkar</p></div>
</div>
...
function displayLogs(logs) {
 if (!logs.length) {
 document.getElementById('logs-list').innerHTML = '<p style="color:#aaa;text-align:center;padding:40px;">Hic лог не найдено</p>';
 return;
 }
 ...
 logs.slice(0,100).forEach(function(лог, i) {
 ...
 html += '<td style="padding:12px;"><code style="color:#aaa;font-size:12px;">'+log.mod_id+'</code></td>';
 html += '<td style="padding:12px;color:#ccc;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+log.reason+'</td>';
 html += '<td style="padding:12px;color:#888;font-size:12px;">'+new Date(log.timestamp).toLocaleString("tr-TR")+'</td>';
 html += '</tr>';
 });
 html += '</table></div><p style="margin-top:15px;color:#666;font-size:13px;">Всего '+logs.length+' islem</p>';
...
function filterLogs() {
 ...
 return String(l.user_id).includes(q) || String(l.mod_id).includes(q) || (l.reason||'').toLowerCase().includes(q) || (l.action||'').toLowerCase().includes(q);
 }));
}
loadLogs();
...
<div class="section">
 ...
 <input type="text" id="warn-filter" placeholder="Filtrele (пользователь ID, moderator, причина...)" style="padding:10px 15px;background:#0a0a0a;border:2px solid rgba(220,20,60,0.3);border-radius:8px;color:#eee;width:300px;" oninput="filterПредупреждениеs()">
 </div>
...
function displayПредупреждениеs(warns) {
 ...
 html += '<td style="padding:12px;color:#888;font-size:12px;">'+new Date(w.timestamp).toLocaleString("tr-TR")+'</td>';
...
function filterПредупреждениеs() {
 ...
 return String(w.user_id).includes(q) || (w.moderator||'').toLowerCase().includes(q) || (w.reason||'').toLowerCase().includes(q);
 }));
}
loadПредупреждениеs();
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(400px,1fr));gap:20px;">
 ...
 <tr style="border-bottom:2px solid rgba(220,20,60,0.3);"><th style="padding:10px;text-align:left;color:#dc143c;">Роль</th><th style="padding:10px;text-align:left;color:#dc143c;">Уровень</th><th style="padding:10px;text-align:left;color:#dc143c;">Администратор</th></tr>
 ...
 <tr style="border-bottom:1px solid rgba(255,255,255,0.05);"><td style="padding:10px;color:#e67e22;font-weight:700;">Иsminin</td><td style="padding:10px;color:#dc143c;">2</td><td style="padding:10px;color:#ccc;">Команды, пользователи, loglar</td></tr>
 <tr><td style="padding:10px;color:#3498db;font-weight:700;">Мод</td><td style="padding:10px;color:#dc143c;">1</td><td style="padding:10px;color:#ccc;">Сервера, loglar, предупреждения</td></tr>
 </table>
 </div>
 <div class="section">
 ...
 <div style="background:rgba(220,20,60,0.1);border:1px solid rgba(220,20,60,0.3);border-radius:10px;padding:15px;">
 <div style="display:flex;justify-content:space-between;align-items:center;">
 <span style="color:#3498db;font-weight:700;">мод</span>
 <span style="background:#3498db;padding:3px 10px;border-radius:10px;font-size:12px;color:white;">MOD</span>
 </div>

...
1. Убедись, что бот запущен с музыкой: в `.env` профиль **`BOT_SLIM=1`**
   (модерация + тикеты + музыка) или верни музыку через
   `EXTRA_COGS=music_cog,voice_commands,voice_tracker`.

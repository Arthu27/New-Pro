...
| `rejoin_roles` | Hakumo — Re-Join Roles (автоматическое восстановление ролей при повторном входе) |
...
| `ticket` | ticket |
| `verification` | Hakumo — Верификация — режим наблюдателя / opt-in |
...
| `replay` | Hakumo — /replay: визуальная лента событий сервера (реплеер инцидентов). |

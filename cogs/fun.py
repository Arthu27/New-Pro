...
class Fun (commands .Cog ):
    ...
    app_commands .Choice (name ="шans",value ="sans"),
    ...
    kullanici1 :discord .Member =None ,kullanici2 :discord .Member =None ):
        ...
        if islem =="zar":
            ...
            bar =""*pct +""*(10 -pct )
            e =discord .Embed (title =" Zar Atыldы!",color =0xDC143C ,timestamp =datetime .utcnow ())
            e .description =(
            f"```ansi\n\u001b[1;31m ZAR SONUCU\u001b[0m\n```\n{_divider()}"
            )
            ...
            (" Шu an belirsiz, tekrar sor.",0xF39C12 ),(" Более после tekrar sor.",0xF39C12 ),
            (" Шu an tahmin edemiyorum.",0xF39C12 ),(" Pek iyi видеть.",0xE74C3C ),
            (" Cevabыm hayыr.",0xE74C3C ),(" Kesinlikle hayыr.",0xE74C3C ),
            ]
            ...
            e .add_field (name =" Ответ",value =f"```{ответ}```",inline =False )
            e .set_footer (text =f"Желание: {interaction.user.name}",icon_url =interaction .user .display_avatar .url )
            await interaction .response .send_message (embed =e )
            ...
            e =discord .Embed (title =" Karar Данныеldi!",color =0xDC143C ,timestamp =datetime .utcnow ())
            e .description =(
            f"```ansi\n\u001b[1;31m KARAR VERИLDИ\u001b[0m\n```\n{_divider()}"
            )
            e .add_field (
            name =f" Выбрать ({len(liste)})",
            value ="\n".join (f"{'' if s == secilen else ''} {s}"for s in liste ),
            inline =False 
            )
            ...
            random .seed ()
            if score >=80 :
                color ,badge ,yorum =0x2ECC71 ," SЮPER ШANSLI","Сегодня каждый что-то yolunda gidecek!"
            elif score >=60 :
                color ,badge ,yorum =0xF1C40F ," ИYИ ШANS","Gюzel bir день seni bekleme."
            elif score >=40 :
                color ,badge ,yorum =0xF39C12 ," ORTA ШANS","Dikkatli ol, ama umudu kesme."
            else :
                color ,badge ,yorum =0xE74C3C ," НИЗКИЙ ШANS","Сегодня temkinli ol!"
            bar =""*(puan //10 )+""*(10 -puan //10 )
            e =discord .Embed (title =" Ежедневный Шans",color =color ,timestamp =datetime .utcnow ())
            e .description =(
            f"```ansi\n\u001b[1;{'32' if очки >= 60 else '33' if очки >= 40 else '31'}m{badge}\u001b[0m\n```\n{_divider()}"
            )
            e .set_thumbnail (url =hedef .display_avatar .url )
            e .add_field (name =" Пользователь",value =hedef .mention ,inline =True )
            e .add_field (name =" Шans Очкиы",value =f"`{bar}` **{очки}/100**",inline =False )
            ...
            score =random .randint (1 ,100 )
            random .seed ()
            kalpler =""*(puan //20 )+""*(5 -puan //20 )
            if score >=80 :
                yorum ,color ="Mюkemmel bir uyum! Ayrыlmayыn! ",0xFF69B4 
            elif score >=60 :
                yorum ,color ="Иyi bir uyum есть, продолжить edin! ",0xE91E8C 
            elif score >=40 :
                yorum ,color ="Центр dюzey uyum, чaba показ! ",0xF39C12 
            else :
                yorum ,color ="Сложный bir iliшki будет... ",0xE74C3C 
            e =discord .Embed (title =" Совместимость Testi",color =color ,timestamp =datetime .utcnow ())
            e .description =(
            f"```ansi\n\u001b[1;35m UYUMLULUK SONUCU\u001b[0m\n```\n{_divider()}"
            )
            e .add_field (name =" Пара",value =f"{пользователь1.mention} & {пользователь2.mention}",inline =False )
            e .add_field (name =" Совместимость",value =f"{kalpler} **%{очки}**",inline =False )
            e .add_field (name =" Комментарий",value =f"*{yorum}*",inline =False )
            e .set_footer (text =f"Aether • {interaction.guild.name}",icon_url =interaction .guild .icon .url if interaction .guild .icon else None )
            await interaction .response .send_message (embed =e )
    ...
    @app_commands .command (name ="profil-kartы",description ="Показать подробную карточку профиля")
    async def profil_karti (self ,interaction :discord .Interaction ,user :discord .Member =None ):
        u =user or interaction .user 
        rozet =""if u ==interaction .guild .owner else ""if u .guild_permissions .administrator else ""if u .guild_permissions .moderate_members else ""
        e =discord .Embed (
        title =f"{rozet} {u.display_name} — Profil",
        color =u .color if u .color .value else 0xDC143C ,
        timestamp =datetime .utcnow ()
        )
        e .description =f"```ansi\n\u001b[1;31m ПОЛЬЗОВАТЕЛЬ PROFИLИ\u001b[0m\n```\n{_divider()}"
        ...
        e .add_field (name =" Platform",value =f"```{'Mobil' if u.is_on_mobile() else 'Masaюstю'}```",inline =True )
    ...
    async def geri_sayim (self ,interaction :discord .Interaction ,секунд :int ,message :str =""):
        if секунд <1 or секунд >60 :
            ...
            return 
        e =discord .Embed (title ="⏱ До Число",color =0xDC143C ,timestamp =datetime .utcnow ())
        e .description =(
        f"```ansi\n\u001b[1;31m⏱ GERИ ЧИСЛО BAШLADI\u001b[0m\n```\n{_divider()}\n\n"
        f"{message}\n\n**{секунд}** секунд kaldы...\n\n{_divider()}"
        )
        await interaction .response .send_message (embed =e )
    ...
    @app_commands .command (name ="hatыrlatыcы",description ="Belirli длительность после seni hatыrlatыr (maks 60 dk)")
    async def hatirlatici (self ,interaction :discord .Interaction ,minutes :int ,message :str ):
        if minutes <1 or minutes >60 :
            ...
            return 
        e =discord .Embed (title ="⏰ Hatыrlatыcы Kuruldu!",color =0x3498DB ,timestamp =datetime .utcnow ())
        e .description =(
        f"```ansi\n\u001b[1;34m HATIRLATICI АКТИВЕН\u001b[0m\n```\n{_divider()}\n\n"
        f"**{minutes} minutes** после seni hatыrlatacaгыm!\n\n{_divider()}"
        )
        e .add_field (name ="⏱ Длительность",value =f"```{minutes} minutes```",inline =True )
        e .add_field (name =" Сообщение",value =f"```{message}```",inline =False )
        e .add_field (name =" Hatыrlatma",value =f"<t:{now_ts() + minutes * 60}:R>",inline =True )
        e .set_footer (text =f"Aether • {interaction.guild.name}",icon_url =interaction .guild .icon .url if interaction .guild .icon else None )
        await interaction .response .send_message (embed =e ,ephemeral =True )
        await asyncio .sleep (minutes *60 )
        try :
            remind_e =discord .Embed (title =" Hatыrlatыcы!",color =0xDC143C ,timestamp =datetime .utcnow ())
            remind_e .description =(
            f"```ansi\n\u001b[1;31m HATIRLATMA!\u001b[0m\n```\n{_divider()}\n\n"
            f"{interaction.user.mention}, bunu hatыrlatmamы желание!\n\n{_divider()}"
            )
...
async def setup (bot ):
    await bot .add_cog (Fun (bot ),guilds =[discord .Object (id =1421244140359909513 ),discord .Object (id =1107038411895881788 ),discord .Object (id =1498837105915330562 )])

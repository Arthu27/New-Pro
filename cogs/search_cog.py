...
class SearchCog(commands.Cog):
    ...
    async def search(self, ctx, *, query: str):
        ...
        embed = discord.Embed(
            title=f" Arama: {query}",
            description="Arama sonuçları загружается...",
    ...
    @commands.command(name='searchuser', aliases=['kullanıcıara'])
    ...
    async def searchrole(self, ctx, *, query: str):
        ...
        for role in roles[:10]:
            embed.add_field(
                name=role.name,
                value=f"ID: {role.id} | Üyeler: {len(role.members)}",
                inline=True
            )
        
        await ctx.send(embed=embed)
    
    @commands.command(name='searchchannel', aliases=['kanalara'])

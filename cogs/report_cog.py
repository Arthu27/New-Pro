...
class ReportCog(commands.Cog):
    ...
    async def report_daily(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=" Daily Report",
            description=f"Tarih: {report['date']}",
    ...
    async def report_weekly(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title=" Weekly Report",
            description=f"Hafta: {report['week_start']} - {report['week_end']}",
            color=discord.Color.blue(),
            timestamp=datetime.now()
        )
    ...
    @app_commands.describe(days='Gün sayısı (varsayılan: 30)', 
    ...
    async def report_custom(self, interaction: discord.Interaction, 
        ...
        if report_type == 'tickets':
            ...
            await interaction.response.send_message(
                " Geçersiz rapor tipi! (tickets/sla/performance)",
                ephemeral=True
            )
            return
    ...
    @app_commands.command(name='report-analytics', description='Analytics raporunu görüntüle')
    async def report_analytics(self, interaction: discord.Interaction):
        """Analytics raporunu görüntüle"""
        # Analytics al

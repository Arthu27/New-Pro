...
class ChangelogCog(commands.Cog):
    ...
    @app_commands.command(name='changelog', description='Changelog\'u görüntüle')
        ...
        if version:
            ...
            if not entries:
                await interaction.response.send_message(
                    f" Versiyon {version} для giriş не найдено!",
                    ephemeral=True
                )
                return
            ...
            embed = discord.Embed(
                ...
                description=f"Toplam {len(entries)} giriş",
            ...
            embed = discord.Embed(
                ...
                description=f"Toplam {len(versions)} versiyon",
                color=discord.Color.blue(),
                timestamp=datetime.now()
            )
    ...
    @app_commands.describe(version='Versiyon', change_type='Değişiklik tipi (added/changed/fixed/removed/security)', 
    ...
    async def changelog_add(self, interaction: discord.Interaction, 
        ...
        # Change type kontrolü
        try:
            change_type_enum = ChangeType(change_type)
        except ValueError:
            await interaction.response.send_message(
                " Geçersiz değişiklik tipi! (added/changed/fixed/removed/security)",
                ephemeral=True
            )
            return
        ...
        # Embed создать
        embed = discord.Embed(
            title=" Changelog Girişi Eklendi",
    ...
    @app_commands.command(name='changelog-latest', description='En son changelog girişlerini görüntüle')
    @app_commands.describe(limit='Giriş sayısı (varsayılan: 10)')
    async def changelog_latest(self, interaction: discord.Interaction, limit: int = 10):
        """En son changelog girişlerini görüntüle"""
        # Girişler al
        ...
        embed = discord.Embed(
            ...
            description=f"Son {len(entries)} giriş",
            color=discord.Color.blue(),
            timestamp=datetime.now()
        )

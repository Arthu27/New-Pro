...
1) Возраст < 18 — НЕЛЬЗЯ ВООБЩЕ (весь сервер).
   • Везде: «мне 16», «я 14 лет», «возраст=17», «четырнадцать»,
     «Ищу девушку мне 16», «ищу парня 16», «16 ищу девушку»,
...
# Знакомства + возраст <18 без «мне»: «ищу парня 16», «16 ищу девушку».
# «Ищу девушку мне 16» уже ловит _UNDERAGE_EXPLICIT_RE; это добивает
# варианты, где возраст стоит рядом с поиском партнёра без «мне/я».
_PARTNER = (
    r'(?:девушк\w*|парн\w*|пацан\w*|мальчик\w*|девочк\w*|'
    r'тиха\w*|тишк\w*|л[юу]д[еия]\w*)'
)
_SEEK = r'(?:ищу|ищем|поищу|познакомл\w*)'
_DATING_UNDERAGE_RE = re.compile(
    rf'(?ix)'
    rf'(?:'
    rf'{_SEEK}[\s\W]{{0,40}}{_PARTNER}'
    rf'[\s\W]{{0,24}}(?:(?:мне|я)\s*)?{_AGE_JUNK}'
    rf'(?P<a_d1>{_UNDER}|[1-9](?!{_AGE_JUNK}\d))(?!\d)'
    rf'{_AFTER}(?:(?:лет|года|год|л\.?){_AFTER})?'
    rf'|'
    rf'(?:^|[^\wа-яё]){_AGE_JUNK}(?P<a_d2>{_UNDER})(?!\d)'
    rf'{_AFTER}(?:(?:лет|года|год){_AFTER})?'
    rf'[\s\S]{{0,24}}?{_SEEK}[\s\W]{{0,24}}{_PARTNER}'
    rf')'
)
...
def claimed_underage(text: str, *, allow_bare: bool = True) -> Optional[int]:
    ...
    if m:
        ...
        if 1 <= age <= AGE_MAX_BLOCK:
            return age
    md = _DATING_UNDERAGE_RE.search(norm)
    if md:
        raw = md.groupdict().get('a_d1') or md.groupdict().get('a_d2')
        age = _digits_to_age(raw)
        if 1 <= age <= AGE_MAX_BLOCK:
            return age
    if allow_bare:

...
class Moderation(commands.Cog):
    ...
    async def moderate_user_error(self, interaction, error):
        if isinstance(error, app_commands.MissingPermissions):
            await interaction.response.send_message(
                embed=error_embed("Yetersiz hakların. Trebuetsya: **Ban участников**."),
                ephemeral=True
            )
        else:
    ...
    async def leave_guild(self, interaction, guild_id: str):
        ...
        if interaction.user.id != app_info.owner.id:
            await interaction.response.send_message(
                embed=error_embed("Eta команда erişimüzerinde только vladelcu botun."),
                ephemeral=True
            )
            return
        try:

...
class Moderation (commands .Cog ):
    ...
    async def _isolate_member (self ,guild ,user ,iso ):
        ...
        апелляции в ЛС боту (кнопка → cogs/appeals.py _submit_appeal):
    ...
    async def _unisolate_member (self ,guild ,user ):
        ...
        # Роль бана сама закрывала каналы — обход всех комнат снова
        # вешает цикл на минуты и «приложение не отвечает».
        if self ._punish_role (guild ,'ban') is not None :
            return
        pool = list (guild .channels )
        for th in getattr (guild ,'threads',None ) or []:
        ...
        if action in ("ban","kick","timeout","mute_chat","untimeout","vmute","vunmute","unmute_chat"):
            ...
            try :
                ...
                try :
                    ...
                    # единственный путь подачи (владелец 2026-09-08)
                    _dm_view =None
                    if action =='ban':
                        try :
            ...
            try :
                ...
                # Настоящий бан снят, человек вне сервера — отправим в ЛС
                # ссылку-возврат (1 использование, 24 часа), если владелец
                # включил разовые инвайты в «Правилах подачи» апелляций.
                _inv_sent =False
                if member is None and unban_done :
                    try :
                        _ap =self .bot .get_cog ('Appeals')
                        if _ap is not None and fetched is not None :
                            from cogs .appeals import settings_of as _ap_settings
                            _inv =await _ap ._make_return_invite (
                                guild ,_ap_settings (_ap ._load (guild .id )))
                            if _inv :
                                await fetched .send (
                                f"🔓 Разбан на сервере «{guild .name }».\n"
                                f"Ссылка-возврат (1 использование, действует сутки):\n{_inv }")
                                _inv_sent =True
                    except Exception as _ie :
                        log .debug (f'[MODPANEL] инвайт возврата: {_ie}')
    ...
    async def on_member_join (self ,member ):
        ...
        try :
            ...
            if not ids :
                return
            ban_role =self ._punish_role (member .guild ,'ban')
            getter =getattr (member .guild ,'get_role',None )
            give =[getter (rid )for rid in ids if callable (getter )]
            give =[r for r in give if r is not None and r !=ban_role
                   and not getattr (r ,'managed',False )]
            if not give :
                return
            await member .add_roles (*give ,reason ='разбан: роли возвращены')
    ...
    def _keep_role_on_ban (self ,guild ,role ,ban_role ):
        ...
        if rid is not None and rid ==getattr (guild ,'id',None ):
            return True
        if ban_role is not None and rid ==getattr (ban_role ,'id',None ):
            return True
        return False

    async def _strip_roles_for_ban (self ,guild ,user ):
        """Бан: забрать все роли (снимок на диск). Разбан отдаст обратно.

        Не трогаем @everyone, managed (буст/бот/интеграция) и саму роль бана.
        Владельца бота сюда не пускаем — бан до этого уже отсечён.
        """
        ...
        ban_role =self ._punish_role (guild ,'ban')
        take =[r for r in roles if not self ._keep_role_on_ban (guild ,r ,ban_role )]
        try :
            from services import punish_roles as PR
            PR .save_held_roles (guild .id ,user .id ,[getattr (r ,'id',0)for r in take ])
        except Exception as _ex :
            log .debug (f'[MODPANEL] save_held_roles: {_ex}')
        if not take :
            return
        reason ='бан: роли сняты до разбана'
        for i in range (0 ,len (take ),10 ):
            chunk =take [i :i +10 ]
            try :
                await user .remove_roles (*chunk ,reason =reason )
            except Exception as _ex :
                log .debug (f'[MODPANEL] strip chunk: {_ex}')
                for r in chunk :
                    try :
                        await user .remove_roles (r ,reason =reason )
                    except Exception as _ex2 :
                        log .debug (f'[MODPANEL] strip {getattr (r ,"name","?")}: {_ex2}')

    async def _restore_roles_after_unban (self ,guild ,member ):
        """Разбан: вернуть роли, снятые при бане."""
        if member is None or guild is None :
            return
        try :
            from services import punish_roles as PR
            ids =PR .take_held_roles (guild .id ,member .id )
        except Exception as _ex :
            log .debug (f'[MODPANEL] take_held_roles: {_ex}')
            return
        if not ids :
            return
        ban_role =self ._punish_role (guild ,'ban')
        ...
        for rid in ids :
            role =getter (rid )if callable (getter )else None
            if role is None or self ._keep_role_on_ban (guild ,role ,ban_role ):
                continue
            give .append (role )
        if not give :
            return
        reason ='разбан: роли возвращены'
        for i in range (0 ,len (give ),10 ):
            chunk =give [i :i +10 ]
            try :
                await member .add_roles (*chunk ,reason =reason )
            except Exception as _ex :
                log .debug (f'[MODPANEL] restore chunk: {_ex}')
                for r in chunk :
                    try :
...
class MuteKindSelect(discord.ui.Select):
    """Второй шаг мута: чат / войс / оба — на ТОЙ ЖЕ панели, потом модалка."""

    def __init__(self, cog, target_id, kinds, panel=None):
...
class MuteKindView(discord.ui.LayoutView):
    ...
    def __init__(self, cog, target_id, kinds, member=None, *, status: str = None,
        ...
        self.member = member
        self.panel = panel
        sel = MuteKindSelect(cog, target_id, kinds, panel=panel)
...
class UnmuteKindSelect(discord.ui.Select):
    """Второй шаг размута: чат / войс / оба — на ТОЙ ЖЕ панели."""
...
async def _resend_fresh_panel(panel):
    """Больше НЕ шлём новое окно — только edit той же панели.

    Оставлено как заглушка: всегда False, чтобы старые вызовы не создавали
    вторую эфемерку.
    """
    log.debug('modpanel resend disabled — только edit той же панели')
    return False
...
async def _enter_kind_mode(interaction, panel, *, kinds, target_id, title,
    ...
    _bind_live_panel(panel, interaction)
    panel.pending_action = None
    panel._kind_mode = 'unmute' if unmute else 'mute'
    panel._kind_target = str(target_id)
    panel._kind_kinds = list(kinds or [])
...
async def _launch_action(cog, interaction, action, prefill, panel=None):
    """Открыть форму / размут. panel — сброс селектов после шага.
    ...
    if action == "mute":
        ...
        if len(kinds) == 1:
            await _offer_mod_form(
                interaction, cog, kinds[0][0], prefill, panel=panel)
            return
        who = prefill
        try:
...
class ModPanelView(discord.ui.LayoutView):
    ...
    def __init__(self, cog, member=None, allowed=None, preselect=None):
        ...
        self._kind_mode = None  # None | 'mute' | 'unmute' — на той же панели
        self._kind_target = None
    ...
    def _clear_kind_mode(self):
        self._kind_mode = None
        self._kind_target = None
        self._kind_kinds = None
        self._kind_title = ''

    def _status_text(self):
        if getattr(self, '_kind_mode', None):
            title = getattr(self, '_kind_title', None) or (
                'Снять мут' if self._kind_mode == 'unmute' else 'Мут')
            uid = getattr(self, '_kind_target', None) or self.selected_uid
            if uid:
                return f'**{title}**\n-# участник <@{uid}> · выберите вид'
            return f'**{title}**\n-# выберите вид'

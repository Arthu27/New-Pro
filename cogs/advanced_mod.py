...
class AdvancedMod(commands.Cog):
    ...
    async def history(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not user_case:
            ...
            e.description = (
                ...
                f"{user.mention} için herhangi bir moderasyon kaydı bulunamadı.\n\n{_divider()}"
            )
            e.set_thumbnail(url=user.display_avatar.url)
            e.add_field(name="👤 Пользователь", value=f"`{user.name}` • `{user.id}`", inline=True)
            e.add_field(name="📊 Статус", value="```diff\n+ Hiç mod действиеi yok\n```", inline=True)
        ...
        e = discord.Embed(title=f"📋  Moderasyon Geçmişi", color=0xE74C3C, timestamp=datetime.utcnow())
        e.description = (
            f"```ansi\n\u001b[1;31m⚠ MOD KAYITLARI\u001b[0m\n```\n{_divider()}"
        )
        e.set_thumbnail(url=user.display_avatar.url)
    ...
    async def case(self, interaction: discord.Interaction, case_id: int):
        ...
        e = discord.Embed(title=f"{emoji}  Case #{case_id} Detayı", color=0x3498DB, timestamp=datetime.utcnow())
        e.description = (
            f"```ansi\n\u001b[1;34m📋 CASE DETAYI\u001b[0m\n```\n{_divider()}"
        )
        if user:
            e.set_thumbnail(url=user.display_avatar.url)
        e.add_field(name="👤 Hedef", value=f"`{user}` • `{case['user_id']}`" if user else f"`{case['user_id']}`", inline=True)
    ...
    async def note(self, interaction: discord.Interaction, user: discord.Member, note: str):
        ...
        e = discord.Embed(title="📝  Not Добавитьndi", color=0xF1C40F, timestamp=datetime.utcnow())
        e.description = (
            f"```ansi\n\u001b[1;33m✔ NOT KAYDEDİLDİ\u001b[0m\n```\n{_divider()}"
        )
        e.set_thumbnail(url=user.display_avatar.url)
        e.add_field(name="👤 Пользователь", value=f"{user.mention}\n`{user.id}`", inline=True)
        e.add_field(name="👮 Добавитьyen", value=interaction.user.mention, inline=True)
    ...
    async def notes(self, interaction: discord.Interaction, user: discord.Member):
        ...
        if not notes:
            await interaction.response.send_message(
                embed=discord.Embed(description=f"📝 {user.mention} için зарегистрированные not yok.", color=0x3498DB),
                ephemeral=True
            )
            return

        e = discord.Embed(title=f"📝  {user.display_name} — Notlar", color=0xF1C40F, timestamp=datetime.utcnow())
        e.description = f"```ansi\n\u001b[1;33m📋 KAYITLI NOTLAR\u001b[0m\n```\n{_divider()}"
        e.set_thumbnail(url=user.display_avatar.url)
    ...
    @app_commands.command(name="watchlist", description="Добавить/убрать из списка наблюдения")
    @app_commands.checks.has_permissions(moderate_members=True)
    async def watchlist(self, interaction: discord.Interaction, user: discord.Member, reason: str = None):
        ...
        if user_id in self.data["watchlist"][guild_id]:
            ...
            self.save_data()
            e = discord.Embed(title="👁️  İzleme Listesinden Çıkarıldı", color=0x2ECC71, timestamp=datetime.utcnow())
            e.description = f"```ansi\n\u001b[1;32m✔ LİSTEDEN ÇIKARILDI\u001b[0m\n```\n{_divider()}"
            e.set_thumbnail(url=user.display_avatar.url)
            e.add_field(name="👤 Пользователь", value=f"{user.mention}\n`{user.id}`", inline=True)
            ...
            self.save_data()
            e = discord.Embed(title="👁️  İzleme Listesine Добавитьndi", color=0xF39C12, timestamp=datetime.utcnow())
            e.description = (
                f"```ansi\n\u001b[1;33m⚠ İZLEMEYE ALINDI\u001b[0m\n```\n{_divider()}\n\n"
                f"{user.mention} artık список наблюденияnde. Hareketleri takip edilecek.\n\n{_divider()}"
            )
            e.set_thumbnail(url=user.display_avatar.url)
            e.add_field(name="👤 Пользователь", value=f"{user.mention}\n`{user.id}`", inline=True)
            e.add_field(name="👮 Добавитьyen", value=interaction.user.mention, inline=True)
    ...
    async def watchlist_show(self, interaction: discord.Interaction):
        ...
        if not watchlist:
            await interaction.response.send_message(
                embed=discord.Embed(description="👁️ Список наблюдения şu an boş.", color=0x3498DB),
                ephemeral=True
            )
            return
        ...
        e.description = (
            f"```ansi\n\u001b[1;33m⚠ İZLEMEDEKİ KULLANICILAR\u001b[0m\n```\n{_divider()}"
        )
    ...
    @app_commands.command(name="banlist", description="Забаненные пользователей listeler")
    ...
    async def banlist(self, interaction: discord.Interaction):
        ...
        if not bans:
            await interaction.followup.send(
                embed=discord.Embed(description="✅ Забаненные user yok.", color=0x2ECC71),
                ephemeral=True
            )
            return

        e = discord.Embed(title=f"🔨  Бан Listesi", color=0xE74C3C, timestamp=datetime.utcnow())
        e.description = (
            f"```ansi\n\u001b[1;31m🔨 BANLI KULLANICILAR\u001b[0m\n```\n{_divider()}"
        )
    ...
    @app_commands.command(name="massrole", description="Массовая выдача/снятие ролей")
            ...
            for member in interaction.guild.members:
                if role not in member.roles and not member.bot:
                    try:
                        ...
                        pass
            e = discord.Embed(title="🎭  Toplu Role Verildi", color=0x2ECC71, timestamp=datetime.utcnow())
            e.description = f"```ansi\n\u001b[1;32m✔ TOPLU ROL\u001b[0m\n```\n{_divider()}"
            e.add_field(name="🎭 Роль", value=role.mention, inline=True)
            e.add_field(name="👥 Etkilenen", value=f"```{count} kişi```", inline=True)
            ...
            for member in interaction.guild.members:
                if role in member.roles:
                    try:
                        ...
                        pass
            e = discord.Embed(title="🎭  Toplu Role Alındı", color=0xE74C3C, timestamp=datetime.utcnow())
            e.description = f"```ansi\n\u001b[1;31m✔ TOPLU ROL ALINDI\u001b[0m\n```\n{_divider()}"
            e.add_field(name="🎭 Роль", value=role.mention, inline=True)
            e.add_field(name="👥 Etkilenen", value=f"```{count} kişi```", inline=True)
        else:
            await interaction.followup.send("❌ Geçersiz действие! `ver` veya `al` kullan.", ephemeral=True)
            return
        e.set_footer(text=f"Aether Moderasyon • {interaction.guild.name}", icon_url=interaction.guild.icon.url if interaction.guild.icon else None)
        await interaction.followup.send(embed=e, ephemeral=True)

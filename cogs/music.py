"""Mюzik botu - yt-dlp + ffmpeg + butonlu player"""
import discord 
...
async def fetch_source (query :str ):
    ...
    with yt_dlp .YoutubeDL (YTDL_OPTS )as ydl :
        try :
            ...
            raise Exception ('Aramama vakit aшыmыna uгradы, tekrar dene')
    if not info :
...
def _build_player_embed (guild_id :int ,title :str ,webpage_url :str ,requester :str ,paused :bool =False )->discord .Embed :
    ...
    mode_text ={'off':' Закрыт','song':' Шarkы','queue':' Очередь'}[mode ]
    q =get_queue (guild_id )
    status ="⏸ Duraklatыldы"if paused else " Чalыyor"
...
class MusicPlayerView (discord .ui .View ):
    ...
    async def stop (self ,interaction :discord .Interaction ,button :discord .ui .Button ):
        ...
        await interaction .response .defer ()
        try :
            e =discord .Embed (title ="⏹ Mюzik Остановитьuldu",color =0x555555 )
    ...
    async def show_queue (self ,interaction :discord .Interaction ,button :discord .ui .Button ):
        ...
        e =discord .Embed (title =" Mюzik Kuyruгu",description =desc ,color =0xdc143c )
...
async def setup (bot ):
    await bot .add_cog (Music (bot ),guilds =Config .guild_objects ())

...
class GamificationCog(commands.Cog):
    ...
    async def leaderboard(self, interaction: discord.Interaction, type: str = 'points'):
        ...
        if type == 'points':
            ...
            await interaction.response.send_message(
                " Geçersiz lider tablosu tipi! (points/badges/level)",
                ephemeral=True
            )
            return
    ...
    @app_commands.command(name='badges', description='Rozetlerinizi görüntüleyin')
    @app_commands.describe(user='Пользователь (opsiyonel)')
    async def badges(self, interaction: discord.Interaction, 
                    user: discord.Member = None):
        """Rozetlerinizi görüntüleyin"""
        target_user = user or interaction.user
        ...
        if not badges:
            await interaction.response.send_message(
                f" {target_user.display_name} henüz hiç rozet kazanmadı!",
                ephemeral=True
            )
            return
        ...
        embed = discord.Embed(
            ...
            description=f"Toplam {len(badges)} rozet",
            color=discord.Color.gold(),
            timestamp=datetime.now()
        )
        ...
        for badge in badges[:10]:
            embed.add_field(
                name=f"{badge['name']}",
                value=f"{badge.get('description', 'Açıklama yok')}\nKazanıldı: {badge['earned_at'][:10]}",
                inline=False
            )
        
        await interaction.response.send_message(embed=embed)
    
    @app_commands.command(name='daily', description='Günlük ödülünüzü alın')
    async def daily(self, interaction: discord.Interaction):
        """Günlük ödülünüzü alın"""
        ...
        if not can_claim:
            ...
            await interaction.response.send_message(
                f"⏰ Günlük ödülünüzü zaten aldınız! {hours}s {minutes}d после tekrar deneyin.",
                ephemeral=True
            )
            return
        ...
        embed = discord.Embed(
            title=" Günlük Ödül",
            description=f"**Kazanılan Очки:** {points}\n\nYarın tekrar gel!",
    ...
    @app_commands.command(name='streak', description='Seri bilgilerinizi görüntüleyin')
    async def streak(self, interaction: discord.Interaction):
        """Seri bilgilerinizi görüntüleyin"""
        ...
        embed.add_field(name=" Mevcut Seri", value=f"{streak_info['current_streak']} gün", inline=True)
        embed.add_field(name=" En Uzun Seri", value=f"{streak_info['longest_streak']} gün", inline=True)
        
        await interaction.response.send_message(embed=embed)
    
    @commands.Cog.listener()
    async def on_message(self, message: discord.Message):
        """Сообщение geldiğinde XP ver"""
        ...
        if xp_gained:
            ...
            embed = discord.Embed(
                title=" Level Up!",
                description=f"**{message.author.mention}** seviye **{level['level']}**'e yükseldi!\n\nYeni seviye: **{level['name']}**",
                color=discord.Color.gold(),
                timestamp=datetime.now()
            )

...
def mod_dm_embed(action, guild, moderator, reason=None, extra_fields=None, gif_key=None):
    ...
    configs = {
        "ban": {
            ...
            "note": "Считаете бан несправедливым? Нажмите кнопку **«Подать апелляцию»** под этим сообщением (или напишите боту команду /апелляция) — после подачи вам откроется канал апелляции на сервере, где модерация её рассмотрит.",
            "gif": None,
        },
        "kick": {
            "title": "👢 Вы кикнуты",

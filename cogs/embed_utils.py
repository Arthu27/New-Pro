...
def mod_dm_embed(action, guild, moderator, reason=None, extra_fields=None, gif_key=None):
    ...
    configs = {
        "ban": {
            ...
            "note": "Считаете бан несправедливым? Нажмите кнопку **«Подать апелляцию»** под этим сообщением (или напишите боту команду /апелляция) — канал апелляции откроется для вас после подачи, а при жёстком бане — сразу после его снятия, и модерация рассмотрит вашу заявку.",
            "gif": None,
        },
        "kick": {
            "title": "👢 Вы кикнуты",

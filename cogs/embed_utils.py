...
def mod_dm_embed(action, guild, moderator, reason=None, extra_fields=None, gif_key=None):
    ...
    configs = {
        "ban": {
            "title": "🚫 Вам выдан бан",
            "color": 0xE74C3C,
            # Владелец 2026-09-08: бан — роль бана, человек остаётся на
            # сервере, доступ закрывает сама роль (каналы бот не обходит).
            "text": f"На сервере **{guild.name}** вам **выдали бан (блокировку)** — каналы сервера для вас закрыты.",
            "note": "Считаете бан несправедливым? Нажмите кнопку **«Подать апелляцию»** под этим сообщением (или напишите боту команду /апелляция) — канал апелляции откроется для вас после подачи заявки, и модерация рассмотрит её.",
            "gif": None,
        },
        "kick": {
            "title": "👢 Вы кикнуты",

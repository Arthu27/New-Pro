...
def mod_dm_embed(action, guild, moderator, reason=None, extra_fields=None, gif_key=None):
    ...
    configs = {
        "ban": {
            "title": "🚫 Вам выдан бан",
            "color": 0xE74C3C,
            # Владелец 2026-09-07: бан — настоящий серверный: человек
            # исключается с сервера и попадает в список банов (изоляции
            # больше нет). Сообщения при этом не удаляются.
            "text": f"На сервере **{guild.name}** вам **выдали бан (блокировку)** — вы исключены с сервера и внесены в список банов.",
            "note": "Считаете бан несправедливым? Нажмите кнопку **«Подать апелляцию»** под этим сообщением (или напишите боту команду /апелляция) — канал апелляции откроется для вас после подачи заявки; при серверном бане вы увидите его сразу после снятия бана и возврата на сервер. Модерация рассмотрит вашу заявку.",
            "gif": None,
        },
        "kick": {
            "title": "👢 Вы кикнуты",
        ...
        "untimeout": {
            ...
            # Нейтрально: мут был и чатовый, и войсовый — про «отправлять
            # сообщения» врать нельзя (войс-мут чат и не трогал).
            "note": "Наказание снято — ограничения убраны. Соблюдайте правила сервера.",
            "gif": None,
        },
        "warn": {

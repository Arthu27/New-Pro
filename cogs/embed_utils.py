...
def mod_dm_embed(action, guild, moderator, reason=None, extra_fields=None, gif_key=None):
    ...
    configs = {
        "ban": {
            ...
            "note": "Считаете бан несправедливым? Нажмите кнопку **«Подать апелляцию»** под этим сообщением — канал апелляции откроется для вас после подачи заявки, и модерация рассмотрит её.",
            "gif": None,
        },
        "kick": {
            "title": "👢 Вы кикнуты",

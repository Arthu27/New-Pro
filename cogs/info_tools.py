...
class InfoTools (commands .Cog ):
    ...
    async def emoji_listesi (self ,ctx ):
        ...
        if not emojis :
            await ctx .send (" Особый emoji нет!")
            return 
        e =discord .Embed (title =" EMOJИ список",color =0xdc143c )
    ...
    async def davet_link (self ,ctx ):
        ...
        e =discord .Embed (title =" BOT DAVETИ",color =0xdc143c )
    ...
    async def vakit (self ,ctx ):
        ...
        e .add_field (name =" Длинный",value =f"<t:{ts}:F>",inline =True )
    ...
    async def banner (self ,ctx ):
        g =ctx .guild 
        if not g .banner :
            await ctx .send ("Bu сервера banneri нет!")
            return 
    ...
    async def role_uyeler (self ,ctx ,*,role :discord .Role ):
        ...
        if not members :
            await ctx .send (f"{role.name} роли кто нет!")
            return 

"""Информация ve arac командаlari — prefix командаlari (!uptime, !botinfo, vb.)"""
import discord
...
class InfoTools(commands.Cog):
    ...
    async def role_info(self, ctx, *, role: discord.Role):
        e = discord.Embed(title=f"🎭 ROL: {role.name}", color=role.color if role.color.value else 0xdc143c)
        ...
        e.add_field(name="📅 Создатьulma", value=f"<t:{int(role.created_at.timestamp())}:R>", inline=True)
        await ctx.send(embed=e)
    ...
    async def channel_info(self, ctx, channel: discord.TextChannel = None):
        ...
        e.add_field(name="📅 Создатьulma", value=f"<t:{int(ch.created_at.timestamp())}:R>", inline=True)
        if ch.topic:
    ...
    async def davet_link(self, ctx):
        ...
        e.description = f"[**➜ Ботu Серверna Добавить**]({url})"
        e.set_thumbnail(url=self.bot.user.display_avatar.url)
        await ctx.send(embed=e)
    ...
    async def color(self, ctx, hex_kod: str):
        ...
        try:
            ...
            await ctx.send("❌ Geçersiz hex kodu! Örnek: `!color ff0000`")
            return
    ...
    async def hesap(self, ctx, *, islem: str):
        ...
        try:
            ...
            await ctx.send("❌ Geçersiz действие!")
    ...
    async def base64_cmd(self, ctx, islem: str, *, metin: str):
        try:
            ...
            await ctx.send("❌ Dönüştürme неудачно!")

    @commands.command(name="banner")
    async def banner(self, ctx):
        g = ctx.guild
        if not g.banner:
            await ctx.send("Bu servernun banneri yok!")
            return
        e = discord.Embed(title=f"{g.name} Банner", color=0xdc143c)
        e.set_image(url=g.banner.url)
        await ctx.send(embed=e)

    @commands.command(name="roleuyeler")
    async def role_uyeler(self, ctx, *, role: discord.Role):
        members = role.members
        if not members:
            await ctx.send(f"{role.name} roleünde kimse yok!")
            return
    ...
    async def ilk_message(self, ctx, channel: discord.TextChannel = None):
        ...
        async for msg in ch.history(limit=1, oldest_first=True):
            e = discord.Embed(title="📌 İLK MESAJ", color=0xdc143c,
                description=f"[Сообщениеa git]({msg.jump_url})\n\n{msg.content[:200] if msg.content else '*(embed)*'}")
            await ctx.send(embed=e)
            return
        await ctx.send("Сообщение bulunamadı!")

...
DIV =""
LOG_CHANNEL_ID =Config .LOG_CHANNEL_ID # Наказание messagelarы bu channela gider
...
class AutoMod (commands .Cog ):
    ...
    async def _get_log_ch (self ,guild ):
        """Наказание messagelarы для log channelыnы вернуть."""
    ...
    async def _purge_user (self ,channel ,member ,limit =5 ):
        """Пользователь son N messageыnы удалить."""
        try :
            deleted =0 
    ...
    async def on_message (self ,message ):
        ...
        # Config yoksa hiчbir что-то yapma
        if not cfg :
            return 
        ...
        if str (message .channel .id )in exempt :
            ...
            # Zapresennoe kelimeler — только panel'de kelime varsa чalышыr
        panel_words =cfg .get ('banned_words',[])
        if panel_words :
            if await self ._check_bad_words (message ,panel_words ):
                return 

                # Spam korumasы — только message удалить, timeout atma
    ...
    async def _check_spam (self ,message ,cfg ):
        ...
        if len (self .spam_tracker [uid ])>=threshold :
            ...
            try :
                ...
                f"{message.author.mention} очень быстрый message atыyorsun, yavaшla!",
                delete_after =5 
                )
            except Exception :
                pass 
    ...
    async def _check_caps (self ,message ):
        ...
        if ratio >0.7 :
            ...
            f"{message.author.mention} aшыrы большой harf использовать!\n** Причина:** CAPS filtresi нарушение",
            0xf39c12 
            )
            return True 
        return False 
    ...
    async def _check_links (self ,message ):
        ...
        message ,
        "  LИNK ENGELLENDИ",
        f"{message.author.mention} link paylaшma администратор нет!\n** Причина:** Link filtresi нарушение",
        0xe67e22 
        )
        return True 
    ...
    async def _check_invites (self ,message ):
        ...
        await self ._punish (
        message ,
        "  DAVET LИNKИ ENGELLENDИ",
        f"{message.author.mention} davet linki paylaшamazsыn!\n** Причина:** Davet filtresi нарушение",
        0xe74c3c 
        )
        return True 
    ...
    # Reply mention'ы sayma (yanыt atarken автоматически 1 mention geliyor)
        ...
        if count >=8 :
            await self ._punish (
            message ,
            "  TOPLU MENTИON ПРЕДУПРЕЖДЕНИЕ",
            f"{message.author.mention} toplu mention zapresenotыr!\n**⏳ Наказание:** 15 minutes mute",
            0xe74c3c ,
            action ='timeout',
            duration_min =15 
            )

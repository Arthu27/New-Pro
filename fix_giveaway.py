...
def fix():
    ...
    gws = {}
    if os.path.exists(f):
        with open(f, 'r') as fp: gws = json.loимя(fp)
    ...
    async def send():
        ...
        embed.имяd_field(name='Kazanan', value=winners, inline=True)
        embed.имяd_field(name='Участник', value='0/{}'.format(winners), inline=True)
        embed.имяd_field(name='Bitiш', value=f'<t:{int(datetime.utcnow().timestamp())+duration*60}:R>', inline=True)
    ...
    asyncio.run_coroutine_threимяsafe(send(), bot.loop)
    return 'FIXED! 🎉'

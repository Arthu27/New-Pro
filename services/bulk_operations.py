...
class BulkUpdater:
    ...
    async def bulk_update_status(self, ticket_ids: List[str], new_status: str,
        ...
        """Массовая состояние güncellemesi"""
        ...
        for ticket_id in ticket_ids:
            try:
                ...
                if ticket.get('status') == 'closed':
                    operation.add_failure(ticket_id, 'Ticket zaten kapalı')
                    continue
        ...
        if exported_tickets:
            ...
            if exported_tickets:
                ...
                with open(filepath, 'w', newline='', encoding='utf-8') as f:
                    ...
                    for ticket in exported_tickets:
                        # Liste ve dict'leri string'e çevir
                        row = {}
...
class BulkImporter:
    ...
    async def bulk_import_json(self, import_data: List[Dict[str, Any]],
        ...
        # Gerçek uygulamada ticket'ları сохранить
        for i, ticket_data in enumerate(import_data):
            try:
                ticket_id = ticket_ids[i]
                # Burada ticket'ı сохранить

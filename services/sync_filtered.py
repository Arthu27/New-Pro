...
# Заказ владельца: в меню «/» должны быть ТОЛЬКО боевые команды — никаких
# настроечных/служебных/setup-команд (вкл/выкл/канал/порог/module/proof
# и пр.), даже для админа. Всё остальное управление — через /modpanel и
# веб-панель. Команды из этого списка живут в Discord; всё прочее
...
# /апелляция убрана (владелец 2026-09-08: «она у нас в кнопке») — подача
# только кнопкой в ЛС/канале; глобальных команд не осталось вовсе,
# старые копии в Discord стираются первым же синком.
...
async def sync_tree(bot, guild=None):
    ...
    # Команды с extras['keep_global'] живут ТОЛЬКО глобально (работают в
    # ЛС). Гильдовая копия = вторая строка с тем же именем в меню сервера.
    # Снимаем такие копии и НЕ возвращаем их в локальное дерево: при
    # следующем guild-sync старые копии, оставшиеся в Discord с прошлых
    # версий, стираются (самолечение). Сейчас keep_global-команд нет:
    # /апелляция удалена, /update стала гильдовой (владелец 2026-09-08).
    if ctx is not None:
        try:
            _kept_global = [
...
def _collect_global_by_keep(tree):
    ...
    for _t in (AppCommandType.chat_input, AppCommandType.user,
        ...
        for cmd in _cmds:
            # Глобально живёт только keep_global ИЗ БЕЛОГО СПИСКА:
            # случайная пометка у небоевой команды не утечёт в ЛС всем
            # (заказ владельца 2026-09-08: «чтобы они исчезли»).
            if _is_keep_global(cmd) and _is_public(cmd):

...
DEFAULT_LIMITS = {
    ...
    # ── опасные операции ──
    'clear': 10,     # сообщений, удалённых очисткой — 10/день У ВСЕХ
                     # (владелец 2026-09-05)
...
# мутов, баны и очистка сообщений. Остальные ключи (kick/vkick/unban/nuke/raid/
# lockdown/dehoist) пока НИГДЕ не считаются — их select'ы в панели были мёртвыми
# дублями выбора и сбивали с толку; DEFAULT_LIMITS их по-прежнему хранит (правила
# не теряются), просто в форму они не выводятся, пока действие не подключим.
ACTION_GROUPS = [
...
TIER_DEFAULT_LIMITS = {
    # warn=1 и clear=10 — единые для всего персонала (владелец 2026-09-05)
    ...
    'owner':   {'warn': 1, 'clear': 10},   # остальное — без ограничений
}
...
def _tier_defaults(role_ids):
    ...
    Для owner — нули по всем действиям (владелец не ограничен), КРОМЕ
    единых для «всех» warn=1 и clear=10 (владелец 2026-09-05: «у всех»).
    Ноль = без лимита и перекрывает модераторский базовый дефолт."""
    tier = tier_for_roles(role_ids)
    if not tier:
        return {}
    if tier == 'owner':
        zeros = {k: 0 for k in DEFAULT_LIMITS}
        zeros.update(TIER_DEFAULT_LIMITS.get('owner') or {})
        return zeros

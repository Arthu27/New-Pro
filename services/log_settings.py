...
LOG_GROUPS = (
    ...
    ('reports', 'Отчёты', '📑',
     'Ветки канала «📑・отчеты»: баны, муты войс/чат, снятие/ЧС стаффа, варны.',
     (
         ('ban',   'Баны', '🔨'),
         ('mute',  'Муты войс / чат', '🔇'),
...
# Канонические имена родительских каналов веток (их создаёт владелец,
# бот не трогает): под «🧵・логи» — ветки событий, под «📑・отчеты» —
# ветки наказаний. Панель показывает имена отсюда (в шаблонах эмодзи
# запрещены — имена приходят переменными).
LOG_PARENT_CHANNELS = {
    'logs': '🧵・логи',
    'reports': '📑・отчеты',
}
...
# «proof» тоже НЕ переносим: у демок своя комната «📸・доказательства»
# (proof_cog зовёт ensure_log_channel('proof')); редирект в «rest»
# отправлял демки в «📋・сервер» и ломал поиск старых proof-каналов.
_DEST = {
    'channel': 'rest',
    'role': 'rest',
    'invite': 'rest',

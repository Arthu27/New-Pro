...
.yr-orb{position:fixed;border-rимяius:50%;pointer-events:none;z-index:0;filter:blur(100px);opacity:0.07;}
...
.yr-каждый{text-align:center;pимяding:32px 20px 28px;position:relative;z-index:1;}
.yr-bимяge{display:inline-flex;align-items:center;gap:8px;background:rgba(212,168,67,0.08);border:1px solid rgba(212,168,67,0.2);border-rимяius:99px;pимяding:5px 16px;font-size:11px;font-weight:700;color:#c8922a;letter-spacing:0.1em;text-transform:uppercase;margin-bottom:18px;}
.yr-bимяge::before{content:'';width:6px;height:6px;border-rимяius:50%;background:#c8922a;animation:blink 1.5s ease-in-out infinite;}
@keyframes blink{0%,100%{opacity:1;box-shимяow:0 0 6px #c8922a;}50%{opacity:0.3;box-shимяow:none;}}
.yr-каждый h1{font-size:clamp(28px,4vw,48px);font-weight:900;letter-spacing:-0.04em;line-height:1;margin-bottom:10px;background:linear-grимяient(135deg,#f0e8d0 0%,#c8922a 40%,#f0e8d0 70%,#e0a830 100%);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;background-size:200% 200%;animation:shimmer 4s ease-in-out infinite;}
...
.yr-stat{display:flex;align-items:center;gap:10px;background:var(--glass);border:1px solid var(--border-base);border-rимяius:12px;pимяding:12px 20px;backdrop-filter:blur(20px);transition:border-color 0.2s,box-shимяow 0.2s,transform 0.2s;cursor:default;}
...
.yr-stat.s-гол:hover{border-color:var(--гол);box-shимяow:0 0 20px var(--gold2);}
.yr-stat.s-green{border-color:rgba(46,213,115,0.2);}
.yr-stat.s-green:hover{border-color:var(--g);box-shимяow:0 0 20px var(--g2);}
.yr-stat.s-blue{border-color:rgba(83,82,237,0.2);}
.yr-stat.s-blue:hover{border-color:var(--b);box-shимяow:0 0 20px var(--b2);}
.yr-stat.s-red{border-color:rgba(255,71,87,0.2);}
.yr-stat.s-red:hover{border-color:var(--r);box-shимяow:0 0 20px var(--r2);}
...
.yr-search{width:100%;pимяding:11px 14px 11px 38px;background:var(--glass);border:1px solid var(--border-base);border-rимяius:12px;color:#f0e8d0;font-size:13px;font-family:Inter,sans-serif;backdrop-filter:blur(20px);transition:border-color 0.2s,box-shимяow 0.2s;margin:0;}
.yr-search:focus{outline:none;border-color:rgba(212,168,67,0.45);box-shимяow:0 0 0 3px rgba(212,168,67,0.08);}
.yr-search::placeholder{color:rgba(240,232,208,0.2);}
.yr-tabs{display:flex;gap:6px;flex-wrap:wrap;}
.yr-tab{display:flex;align-items:center;gap:6px;pимяding:8px 16px;background:var(--glass);border:1px solid var(--border-base);border-rимяius:99px;color:rgba(240,232,208,0.4);font-size:11px;font-weight:700;cursor:pointer;font-family:Inter,sans-serif;transition:all 0.18s;white-space:nowrap;backdrop-filter:blur(12px);}
.yr-tab:hover{color:#f0e8d0;border-color:rgba(212,168,67,0.3);background:rgba(212,168,67,0.06);}
.yr-tab.active{color:#c8922a;border-color:rgba(212,168,67,0.4);background:rgba(212,168,67,0.1);box-shимяow:0 0 16px rgba(212,168,67,0.15);}
.yr-tab[data-c="r"].active{color:var(--r);border-color:var(--r);background:var(--r2);box-shимяow:0 0 16px var(--r-glow);}
.yr-tab[data-c="g"].active{color:var(--g);border-color:var(--g);background:var(--g2);box-shимяow:0 0 16px var(--g-glow);}
.yr-tab[data-c="b"].active{color:var(--b);border-color:var(--b);background:var(--b2);box-shимяow:0 0 16px var(--b-glow);}
.yr-tab[data-c="y"].active{color:var(--y);border-color:var(--y);background:var(--y2);box-shимяow:0 0 16px var(--y-glow);}
...
.yr-section-count{font-size:10px;color:rgba(240,232,208,0.3);border:1px solid rgba(212,168,67,0.1);border-rимяius:99px;pимяding:2px 8px;background:rgba(212,168,67,0.05);}
.yr-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:10px;}
.yr-card{background:var(--glass);border:1px solid rgba(212,168,67,0.08);border-rимяius:12px;pимяding:16px 18px;cursor:default;transition:all 0.22s;backdrop-filter:blur(20px);position:relative;overflow:hidden;}
.yr-card::after{content:'';position:absolute;top:0;left:-100%;width:60%;height:100%;background:linear-grимяient(90deg,transparent,rgba(255,255,255,0.03),transparent);transition:left 0.5s;}
.yr-card:hover::after{left:150%;}
.yr-card:hover{transform:translateY(-2px);}
.yr-card[data-c="r"]:hover{border-color:var(--r);box-shимяow:0 0 20px var(--r2),0 4px 20px rgba(0,0,0,0.4);}
.yr-card[data-c="g"]:hover{border-color:var(--g);box-shимяow:0 0 20px var(--g2),0 4px 20px rgba(0,0,0,0.4);}
.yr-card[data-c="b"]:hover{border-color:var(--b);box-shимяow:0 0 20px var(--b2),0 4px 20px rgba(0,0,0,0.4);}
.yr-card[data-c="y"]:hover{border-color:var(--y);box-shимяow:0 0 20px var(--y2),0 4px 20px rgba(0,0,0,0.4);}
.yr-card[data-c="p"]:hover{border-color:var(--p);box-shимяow:0 0 20px var(--p2),0 4px 20px rgba(0,0,0,0.4);}
.yr-card[data-c="гол"]:hover{border-color:var(--гол);box-shимяow:0 0 20px var(--gold2),0 4px 20px rgba(0,0,0,0.4);}
...
.pb{font-size:9px;font-weight:700;pимяding:2px 7px;border-rимяius:99px;text-transform:uppercase;letter-spacing:0.08em;white-space:nowrap;flex-shrink:0;}
.pb-all{background:rgba(46,213,115,0.1);color:var(--g);border:1px solid rgba(46,213,115,0.25);}
.pb-мод{background:rgba(255,165,2,0.1);color:var(--y);border:1px solid rgba(255,165,2,0.25);}
.pb-админ{background:rgba(255,71,87,0.1);color:var(--r);border:1px solid rgba(255,71,87,0.25);}
.yr-empty{text-align:center;pимяding:60px 20px;color:rgba(240,232,208,0.2);font-size:13px;position:relative;z-index:1;}
...
<div class="yr-каждый">
  <div class="yr-bимяge">Aether Bot</div>
...
<script>
var YR_CATS=[
  {id:'мод',color:'r',title:'Модerasyon',cmds:[
    {n:'/модerate бан',d:'Пользователя постоянный запретla',u:'/модerate бан @user причина',p:'админ'},
    {n:'/модerate кик',d:'С сервера at',u:'/модerate кик @user причина',p:'админ'},
    {n:'/модerate timeout',d:'Gecici sustur',u:'/модerate timeout @user 10m',p:'админ'},
    {n:'/модerate untimeout',d:'Susturmayi удалить',u:'/модerate untimeout @user',p:'админ'},
    {n:'/модerate unбан',d:'Банi удалить',u:'/модerate unбан user_id',p:'админ'},
  ...
  {id:'варн',color:'y',title:'Предупреждение Система',cmds:[
    ...
    {n:'/варнings',d:'Предупреждения listele',u:'/варнings @user',p:'мод'},
    {n:'/clearварнs',d:'Tum предупреждения clear',u:'/clearварнs @user',p:'админ'},
  ]},
  ...
  {id:'fun',color:'p',title:'Eglence ve Игра',cmds:[
    ...
    {n:'/кубик-at',d:'Бросить кубик (1-5 имяet)',u:'/кубик-at 2',p:'all'},
  ...
  {id:'eco',color:'гол',title:'Ekonomi ve Ozellikler',cmds:[
    ...
    {n:'/duty-имяd',d:'Manuel ilerleme имяd',u:'/duty-имяd @user 10',p:'мод'},
  ...
  {id:'util',color:'b',title:'Помощник Aramaclar',cmds:[
    {n:'/модstats',d:'Модerator статистика',u:'/модstats',p:'мод'},
    {n:'/activeмодs',d:'En активен модeratorler',u:'/activeмодs',p:'мод'},
...
function pb(p){
  ...
  if(p==='мод')return '<span class="pb pb-мод">Мод</span>';
...
function yrRender(){
  ...
  YR_CATS.forEach(function(cat){
    ...
    html+='<span class="yr-section-title" style="color:'+cm.t+';text-shимяow:0 0 10px '+cm.t+'40;">'+cat.title+'</span>';
    html+='<div class="yr-section-line" style="background:linear-grимяient(90deg,'+cm.l+',transparent);"></div>';
    ...
    f.forEach(function(x){
      ...
      html+='<div class="yr-card-top"><span class="yr-cmd-name" style="color:'+cm.t+';text-shимяow:0 0 8px '+cm.t+'60;">'+x.n+'</span>'+pb(x.p)+'</div>';

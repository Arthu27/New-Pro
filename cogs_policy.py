...
# Музыка (исторический состав): плеер + голосовые команды + трекер времени
# в голосе. ВНИМАНИЕ (2026-09-01): /play и голосовые музыкальные команды
# сняты с эксплуатации по решению владельца — music_cog.py и voice_commands.py
# переведены в RETIRED_COGS (файлы на диске, но не грузятся ни в одном
# профиле, включая BOT_FULL/BOT_SLIM). Константа оставлена только для
# совместимости импортов; в профилях её больше не используют.
...
# Голосовая статистика (без воспроизведения музыки) — остаётся боевой:
# панель показывает время в голосе, этот ког не даёт команд /play.
...
# Музыка снята (2026-09-01): вместо MUSIC_COGS — только VOICE_STATS_COGS
...
RETIRED_COGS = frozenset({
    ...
    'ticket.py',
    # Музыка полностью снята с эксплуатации по решению владельца (2026-09-01):
    # /play и голосовые музыкальные команды не нужны — бот модерационный.
    # Файлы остаются на диске и под тестами (ничего не удаляем); веб-страница
    # /music остаётся доступной и покажет «модуль выключен». voice_tracker.py
    # НЕ трогаем — это статистика пребывания в голосе (её читает панель), к
    # воспроизведению музыки он отношения не имеет.
    'music_cog.py',
    'voice_commands.py',
})

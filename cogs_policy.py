...
музыка, AI-модерация и приветствия. AI-чат снят с эксплуатации (RETIRED).
Вся «веселуха» (экономика, игры, левелинг, ивенты, соц-системы…) УСЫПЛЕНА:
файлы и данные на месте, модуль просто спит. Вернуть всё — .env:
`BOT_FULL=1`, вернуть точечно — `EXTRA_COGS` (AI-чат — нет, он в RETIRED).
...
Ручки управления через .env, без правки кода:
    ...
    MOD_ONLY=1            — «только модерация»: грузятся модераторские и
                            ...
                            музыка, ивенты, левелинг…) выключена.
    ...
    BOT_SLIM=1            — «модерация + заявки + голосовая статистика»:
                            грузятся ядро, модераторские модули, заявки
                            в команду и voice_tracker; музыка (/play) и
                            AI-чат сняты. Остальная «веселуха» (экономика,
                            игры, ивенты, левелинг…) выключена. Данные на
                            диске остаются — вернёшь флаг, модули вернутся.

    BOT_CORE=1            — «модерация + тикеты + логи» (ядро без музыки
                            и без AI-чата): грузятся системные, модераторские,
                            тикетные и лог-модули. Экономика, игры, музыка,
                            левелинг и прочая «веселуха» выключена.
...
# AI-чат снят с эксплуатации (RETIRED_COGS). Константа оставлена пустой,
# чтобы старые импорты/тесты не ломались на имени AI_CHAT_COGS.
AI_CHAT_COGS = frozenset()

# Профиль «модерация + заявки + голосовая статистика» (музыка и AI-чат сняты).
SLIM_COGS = CORE_COGS | MODERATION_COGS | TICKET_COGS | VOICE_STATS_COGS

# Профиль «ядро»: модерация + тикеты + логи (без музыки и без AI-чата).
# Логи (logs.py, log_menu.py) уже в MODERATION_COGS, AI-модерация — тоже.
CORE_ONLY_COGS = CORE_COGS | MODERATION_COGS | TICKET_COGS
...
# AI: только модерация токсичности (чат-ассистент снят — RETIRED_COGS).
...
RETIRED_COGS = frozenset({
    ...
    #
    # AI-чат (Discord-канал + панель /ai-chat) снят с эксплуатации
    # (заказ владельца 2026-09-22): ког не грузится ни в одном профиле,
    # пункт меню и маршруты панели отключены. AI-модерация токсичности
    # (ai_moderation.py) остаётся. Файлы на диске пока лежат — снести
    # можно отдельной чисткой.
    'ai_chat.py',
})

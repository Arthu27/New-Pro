@echo off
rem ============================================================================
rem  Aether Panel — подключение личного домена panel.hakumods.xyz
...
cd /d "%~dp0"
title Aether Panel — туннель на домен

set DOMAIN=panel.hakumods.xyz
set TNAME=aether-panel
set PANEL_PORT=5001

echo.
echo  ==========================================================
echo   Aether Panel — личный домен
echo   %DOMAIN%  =^>  http://localhost:%PANEL_PORT%
echo  ==========================================================
echo.
echo  Перед стартом убедись, что:
echo   1) hakumods.xyz добавлен в Cloudflare и статус "Active"
echo   2) бот/панель сейчас запущены (start.bat)
echo.
pause
...
echo.
echo [2/6] Вход в Cloudflare. Сейчас откроется браузер — выбери сайт hakumods.xyz
...
rem --- 4. привязка домена -----------------------------------------------------
echo.
echo [4/6] Привязываю %DOMAIN% к туннелю (создаю CNAME в Cloudflare)...
cloudflared.exe tunnel route dns %TNAME% %DOMAIN%
if errorlevel 1 (
  echo [ОШИБКА] Не вышло привязать домен. Проверь, что hakumods.xyz в Cloudflare "Active".
  pause & exit /b 1
)
...
if errorlevel 1 (
  ...
  echo.
  echo  ==========================================================
  echo   ГОТОВО! Туннель работает и будет сам стартовать с ПК.
  echo.
  echo   Панель домена:  https://%DOMAIN%
  echo.
  echo   Осталось: в .env бота допиши  WEB_BEHIND_PROXY=1

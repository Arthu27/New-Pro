...
сервер, и beforehand делает резервную копию data\.
...
def apply_plan(actions: list) -> dict:
    ...
    for kind, target, _desc in actions:
        try:
            if kind == 'file':
                ...
                path, gid = target
                touched_json.setdefault(path, set()).add(gid)
            elif kind == 'db_row':
                db, gid = target
                con = sqlite3.connect(db)
    ...
    for path, gids in touched_json.items():
        try:
            with open(path, encoding='utf-8') as f:
                data = json.load(f)
            for gid in gids:

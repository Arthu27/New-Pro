...
Читает очередь и состояние голосового клиента прямо из когa (in-memory),
мутации делают те же операции, что и кнопочный пульт под /play: pause/resume/
stop голосового клиента, чистые shuffle_queue/remove_track из cogs.music_cog,
очистка очереди — тем же присвоением, что и кнопка «Стоп».
(Отдельные музыкальные команды бота удалены по решению владельца: в Discord
остался только /play с автоматическим пультом кнопок; веб-пульт показывает
базу.)

Чтение (страница и состояние) — mod+. Управление — admin+.
"""
import asyncio

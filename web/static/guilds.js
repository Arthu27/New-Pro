fetch('/api/guilds', { credentials: 'same-origin' })
    ...
    .then(function(guilds) {
        if (!guilds || guilds.length === 0) {
            document.getElementById('guilds-container').innerHTML = '<p style="color: #aaa;">Henüz sunucu yok</p>';
            return;
        }
        ...
        guilds.forEach(function(g) {
            ...
            html += '<i class="fas fa-users"></i> ' + g.members + ' üye';
            html += '</div></div>';
        });
    ...
    .catch(function(error) {
        ...
        document.getElementById('guilds-container').innerHTML = '<p style="color: #e74c3c;">Sunucular yüklenirken hata: ' + error.message + '</p>';
    });

...
Otslejivanie duygular, tona, çakışmaov
"""
import discord
import json
import os
...
class SentimentAnalyzer:
    """Analizör duygu сервер"""
    
    # Kalıplar для opredeleniya duygular
    EMOTION_PATTERNS = {
        'positive': [
            r'\b(teşekkürler|blagodaryu|mükemmel|kruto|süper|klass|zdorovo|prekrasno|zamesohbettelno)\b',
            r'\b(iyi|normal|ok|oky|ladno|ponyal|prinyal)\b',
        ...
        'negative': [
            ...
            r'\b(kötü|ujasno|otvratitelno|kosmar|jvar)\b',
    ...
    def analyze_message(self, message: discord.Message) -> Dict:
        ...
        result = {
            ...
            'content': message.content[:200],  # Ограничиваем uzunluğu
            'emotions': emotions,
    ...
    def get_channel_sentiment(self, channel_id: int, window_minutes: int = 60) -> Dict:
        """Alıyor duygu канал для son N dakika"""
    ...
    def get_sunucu_sentiment(self, guild: discord.Guild, window_minutes: int = 60) -> Dict:
        """Alıyor общий duygu сервер"""
        channel_sentiments = []
        ...
        # Hesaplıyoruz ortalama по на сервер
    ...
    async def check_for_alerts(self, guild: discord.Guild) -> List[Dict]:
        """Контроль ediyor nujni mı предупреждение о nastroenii"""
        alerts = []
        ...
        for channel in guild.text_channels:
            ...
            # Предупреждение если çakışma (очень negativa для korotkoe время)
            ...
            if recent_10min['emotion_breakdown']['negative'] >= 5:
                ...
                if alert_key not in self.alerts_sent:
                    alerts.append({
                        ...
                        'message': f"🔥 Vozmojniy çakışma в #{channel.name} ({recent_10min['emotion_breakdown']['negative']} negativnih сообщение)"
                    })
                    self.alerts_sent.add(alert_key)
...
def get_sentiment_analyzer() -> SentimentAnalyzer:
    """Alıyor küresel пример SentimentAnalyzer"""

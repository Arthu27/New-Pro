...
def _local_hakumo_fallback (messages :List [Dict ])->Tuple [str ,str ,Dict ]:
    ...
    # 1. Приветствие
    if any (k in q_lower for k in ["привет","здравствуй","хай","салют","доброе утро","добрый день","добрый вечер","hey"]):
        return (
        "Привет! Я **Hakumo** — AI-ассистент этого Discord-сервера.\n"
        "Отвечаю на вопросы по правилам, командам, панели и состоянию сервера. "
        "Наказания выдаёт только модератор-человек — я помогаю разобраться. Чем помочь?",
        _tag ,_meta 
        )

    # 1.05. Кто ты / что это (Hakumo) — раньше общих блоков
    ...
    if _who or _what_bot :
        return (
        "Я **Hakumo** — информационный AI-ассистент этого Discord-сервера.\n"
        "• Знаю правила, каналы, роли, онлайн и справку по панели.\n"
        "• Подсказываю команды (`/modpanel`, `/report`, `/afk` и др.).\n"
        "• Не выдаю наказания сам — это делают модераторы-люди.\n\n"
        "Напиши **«команды»**, **«правила»** или свой вопрос — отвечу по делу.",
    ...
    if any (k in q_lower for k in ["как дела","как жизнь","что нового","как ты","как самочувствие"]):
        return (
        "У меня всё отлично! Системы сервера под контролем.\n"
        "А у тебя как? Нужны правила, команды, статус сервера или помощь с панелью?",
        _tag ,_meta 
        )

    # 3. (блок «кто ты» перенесён выше — сразу после приветствия)

    # 4. Спасибо
    if any (k in q_lower for k in ["спасибо","спс","благодарю","сяп","thank"]):
        return (
        "Пожалуйста! Если ещё что-то нужно по серверу или панели — спрашивай.",
        _tag ,_meta 
        )
    ...
    if re .search (r'\bпока\b|до свидания|\bудачи\b|спокойной ночи|до встречи|\bбывай\b',q_lower ):
        return (
        "До встречи! Я на связи — пиши, когда понадобится помощь по серверу.",
        _tag ,_meta 
        )
    ...
    if any (k in q_lower for k in ["команда","помощь","help","особенность","команды","что ты умеешь","справка","какие команды"]):
        return (
        "**Команды Hakumo:**\n"
        "• **Модерация:** `/modpanel` — варн, таймаут, мут, кик, бан; `/report` — жалоба; `/my-violations` — свои наказания.\n"
        "• **Апелляции:** кнопка в карточке бана / канале / через `/my-violations`.\n"
        "• **Прочее:** `/afk` — отойти; `/update` — обслуживание (владелец).\n"
        "Музыки и экономики в боте нет — это модерационный ассистент.\n"
        "Спроси про правила, онлайн или раздел панели — отвечу точнее.",
        _tag ,{**_meta ,"latency_ms":11 }
        )
    ...
    if any (k in q_lower for k in ["предупреждение","варн","наказание","бан","кик","мут","история наказа"]):
        return (
        "**Модерация:**\n"
        "• Варны, баны, кики и муты ведёт система модерации бота.\n"
        "• История: `/history @пользователь` или панель → логи / пользователи.\n"
        "• Жалоба: `/report` или тикет с доказательствами.\n"
        "Наказания выдаёт человек-модератор, не я.",
        _tag ,{**_meta ,"latency_ms":14 }
        )
    ...
    if any (k in q_lower for k in ["ticket","поддержка","тикет","жалоба","администратор","админ","проблема","модератор","помогите"]):
        ...
        "**Поддержка:**\n"
        "• Жалоба — `/report`.\n"
        "• Вопросы по правилам и командам можно задать мне здесь.\n"
        "• Сложные случаи — модераторам через тикет или канал поддержки.\n"
        "Опиши проблему коротко — подскажу, куда идти.",
        _tag ,_meta 
        )

    # 20. Умный дефолт
    short =last_msg [:160 ].replace ('\n',' ')
    hints =[]
    if 'ПРАВИЛА СЕРВЕРА'in sys_prompt or 'Правило #'in sys_prompt :
        hints .append ("напиши **«правила»**")
    hints .append ("напиши **«команды»**")
    hints .append ("спроси про **онлайн**, **панель** или конкретного участника")
    return (
    f"Понял сообщение: «{short}».\n\n"
    "Я **Hakumo** — отвечаю по серверу и панели. "
    "Сейчас нет ответа от AI-сервиса, поэтому краткая подсказка:\n"
    "• "+", ".join (hints )+".\n"
    "• Сформулируй вопрос конкретнее (например: «какие правила?», «кто в онлайне?», «как открыть modpanel?») — отвечу точнее.\n"
    "Если нужен живой AI — проверьте ключ `MISTRAL_API_KEY` в настройках бота.",
    _tag ,{**_meta ,"latency_ms":12 }
    )
...
def _call (messages :List [Dict ],max_tokens :int =2048 ,temperature :float =0.7 ,model :str =None )->Tuple [str ,str ,Dict ]:
    ...
    4) Локальный офлайн-движок Hakumo (fallback)
    """
...
temperature :float =None ,max_tokens :int =None ,model :str =None )->Tuple [str ,List [Dict ],str ,Dict ]:
    ...
    sys_lines =[
    "Ты — Hakumo, информационный AI-ассистент сервера Discord. Отвечай ТОЛЬКО на русском языке. Никакого турецкого.",

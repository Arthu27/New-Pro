...
- /replay [@user] [минуты] — визуальная лента событий сервера (карточка-таймлайн): заходы, удаления, роли, муты
- /ladder — лестница авто-наказаний по предупреждениям (карточка)
- /ladder-add N mute|kick|ban [длительность] [единица] — добавить ступень лестницы (менеджер)
- /ladder-remove N — убрать ступень
- /ladder-test @user — что грозит участнику сейчас
...
- Куратор — старший модератор (всё модерское + тикеты и сообщество), настраивается владельцем в «Панели и роли» и «Меню панели» (вкладка «Куратор»)

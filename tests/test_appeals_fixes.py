# -*- coding: utf-8 -*-
"""Апелляции — правки по жалобе владельца 2026-09-06 («проверь сам лично»).

1) Дубликаты заявок: пока апелляция на рассмотрении, новая от того же
   человека не проходит — одна заявка, одна карточка.
2) Кнопка «Взять в работу» (Discord и панель) открывает канал апелляции
   забаненному участнику; снятие с работы канал не трогает.
3) После решения карточка в канале удаляется: сообщение, собственная
   ветка и пинг роли.
4) Принятие апелляции пишет дело «unban» и карточку «Блокировка снята»
   с автором решения; если аудит промолчал, логи всё равно называют
   модератора из дела (_actor_person/_actor_short), а не «—».

Запуск: python3 tests/test_appeals_fixes.py
"""
import asyncio
import json
import os
...
GID = 611000
BANNED = 697389321447145482          # Егорик из примера владельца
...
st = AP.empty_state()
first, err = AP.create_appeal(st, BANNED, 'Егорик',
                              'Разбаните пожалуйста, это было недоразумение', NOW)
check(first is not None and err is None, 'первая апелляция принята', err or '')
dup, err = AP.create_appeal(st, BANNED, 'Егорик',
                            'Ещё раз прошу, разбаньте уже', NOW)
check(dup is None and f'#{first["id"]}' in err and 'рассмотрении' in err,
      'дубль отклонён с номером живой заявки', err or '')
check(len(AP.pending_items(st)) == 1, 'в очереди всё ещё одна заявка')
# другой человек подаёт свободно — дедуп не мешает чужим заявкам
other, err = AP.create_appeal(st, 111222333444555666, 'Другой',
                              'Мой бан был за спам, каюсь', NOW)
check(other is not None and other['id'] != first['id'],
      'другому человеку дедуп не мешает')
# после решения слот свободен (кулдаун обходим временем)
AP.resolve_appeal(st, first['id'], False, 'Мод', NOW, reply='нет')
after, err = AP.create_appeal(
    st, BANNED, 'Егорик', 'Подаю заново после отказа',
    NOW + timedelta(hours=AP.DEFAULT_COOLDOWN_HOURS + 1))
check(after is not None, 'после решения подать снова можно', err or '')


print('== 2. «Взять в работу» открывает канал апелляции (Discord) ==')
...
class _FetchedUser:
    def __init__(s, uid):
        s.id = uid
        s.name = 'Егорик'
        s.display_name = 'Егорик'


class _FakeGuild:
    def __init__(s, gid):
        s.id = gid
        s.name = 'Тестовый сервер'
...
check(opened_calls == [(GID, 111222333444555666)],
      f'клик «Взять в работу» открыл канал забаненному {opened_calls}')
check(any('канал апелляции открыт' in n.lower() for n in it.followup.notes),
      'модератору видно, что канал открыт', str(it.followup.notes)[:100])
check(other['claimed_by'] and str(other['claimed_by']['id']) == str(MOD),
      'заявка помечена «в работе» у нажавшего')
# повторный клик того же мода = снять с работы: канал НЕ открываем заново
opened_calls.clear()
it2 = _Inter(mod_user)
...
class _ChanGuild(_FakeGuild):
    ...
    def get_channel_or_thread(s, cid):
        return s.channels.get(cid)
...
cog2._guild_ch = lambda g, cid: (cg.get_channel_or_thread(cid)
                                 if g is not None else None)

# 3a. карточка в собственном треде: тред удаляется целиком
item_t = {'id': 21, 'user_id': BANNED, 'thread_id': 701,
          'card_channel_id': 700, 'message_id': 9001,
          'ping_message_id': 9002}
ok = asyncio.new_event_loop().run_until_complete(
    cog2._delete_appeal_card(cg, {}, item_t))
check(ok and thread.deleted and not log_chan.deleted,
      'карточка в своей ветке — ветка удалена целиком, канал цел')
check(9002 in log_chan.fetched, 'пинг роли под карточкой тоже удалён')

# 3b. карточка без треда: удаляем сообщение по сохранённым id
log_chan2 = _FakeChan(700)
cg.channels[700] = log_chan2
item_m = {'id': 22, 'user_id': BANNED, 'card_channel_id': 700,
          'message_id': 9101, 'ping_message_id': 9102}
ok = asyncio.new_event_loop().run_until_complete(
    cog2._delete_appeal_card(cg, {}, item_m))
check(ok and 9101 in log_chan2.fetched and 9102 in log_chan2.fetched,
      'карточка и пинг удалены по id из панели')

# 3c. клик по кнопке: сообщение из интеракции удаляется напрямую
msg = _Msg()
item_c = {'id': 23, 'user_id': BANNED}
ok = asyncio.new_event_loop().run_until_complete(
    cog2._delete_appeal_card(None, {}, item_c, message=msg))
check(ok and msg.deleted, 'карточка из клика удалена через message.delete')

# 3d. ничего нет — тихо возвращаем False, без падения
ok = asyncio.new_event_loop().run_until_complete(
    cog2._delete_appeal_card(None, {}, {'id': 24, 'user_id': BANNED}))
...
async def _fake_send(guild, action, user, moderator, **kw):
    sent_cards.append((action, getattr(user, 'id', None),
                       getattr(moderator, 'id', None), kw.get('reason'),
                       kw.get('case_id')))
    return True
...
case_id = asyncio.new_event_loop().run_until_complete(
    cog3._log_unban_decision(guild_with_member, item_a, MOD, 'Модератор Егор'))
check(case_id == 555 and saved_cases and saved_cases[0][1] == 'unban'
      and saved_cases[0][2] == BANNED and saved_cases[0][3] == MOD,
      'дело «unban» записано от имени принявшего модератора', str(saved_cases))
check(saved_cases[0][4] == 'Апелляция #31 принята',
      'причина дела — принятие апелляции')
check(len(sent_cards) == 1 and sent_cards[0][0] == 'unban'
      and sent_cards[0][1] == BANNED and sent_cards[0][2] == MOD
      and sent_cards[0][4] == 555,
      'карточка «Блокировка снята» ушла с автором и делом', str(sent_cards))
LOGS.send_action_log = _real_send


print('== 5. Логи без аудита всё равно называют модератора ==')
# свежее дело «unban» по Егорику: аудита нет (who=None) — берём из дела
case = {'user_id': str(BANNED), 'action': 'unban', 'mod_id': str(MOD),
        'mod_name': 'Модератор Егор',
...
LOGS._JSON_FILE_CACHE.clear()   # файл новый — кэш по mtime сбрасываем руками
gm = _ChanGuild(GID)          # get_member → None: имя возьмётся из дела
line = LOGS._actor_person(None, guild=gm, target_id=BANNED,
                          actions=('unban',))
check('Модератор Егор' in line, 'кто снял бан — из дела, не «—»', line[:80])
short = LOGS._actor_short(None, guild=gm, target_id=BANNED,
                          actions=('unban',))
check(short == 'Модератор Егор', 'короткое имя тоже из дела', short)
# дела нет — честное «—»
no_case_guild = _ChanGuild(999888)
check(LOGS._actor_person(None, guild=no_case_guild, target_id=BANNED,
                         actions=('unban',)) == '—',
      'без дела и аудита — прежнее «—»')


print('== 6. Панель: claim открывает канал, resolve чистит карточку ==')
import web.app as appmod  # noqa: E402
from web.routes import appeals_panel as SP  # noqa: E402

# живой loop в фоне — _run_async работает как с настоящим ботом
...
class _LoopBot(_FakeBot):
    ...
    def get_cog(s, name):
        return s._stub if name == 'Appeals' else None
...
# claim из панели → канал открыт забаненному
st2 = AP.empty_state()
pend, _e = AP.create_appeal(st2, BANNED, 'Егорик',
                            'Панельная заявка — прошу разбана', NOW)
SP._save(GID, st2)
opened = SP._open_channel_for_claim(loop_bot, GID, pend)
check(opened is True and stub.opened == [BANNED],
      f'claim из панели открыл канал: {stub.opened}')
...
SP._save(GID, res_st)
ok, err, code, payload = SP.resolve_panel(loop_bot, GID, acc['id'], True,
                                          'ПанМод', reviewer_id=MOD)
check(ok and err == '', 'решение из панели прошло', err)
check(stub.unban_cards and stub.unban_cards[0][0] == acc['id']
...
async def _boom_channel(guild, category='сервер'):
    raise _Boom()          # добрались до отправки карточки — фиксируем
...
def _run_unban():
    return asyncio.new_event_loop().run_until_complete(
        probe.on_member_unban(gm, u))


# свежее дело «unban» = разбаняли мы (кнопка/панель): карточку уже sends
# наш маршрут — слушатель молчит, дубля нет
case_fresh = {'user_id': str(BANNED), 'action': 'unban', 'mod_id': str(MOD),
              'mod_name': 'Модератор Егор',
              'timestamp': datetime.now(UTC).isoformat()}
with open(os.path.join('data', 'mod_data.json'), 'w', encoding='utf-8') as fh:
...
# дела нет — слушатель честно строит свою карточку (доходи до канала)
with open(os.path.join('data', 'mod_data.json'), 'w', encoding='utf-8') as fh:

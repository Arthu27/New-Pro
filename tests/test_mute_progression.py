# -*- coding: utf-8 -*-
"""Потолок мута без прогрессии «+2» (отменена 2026-09-25).
...
print('== 1. Фиксированный потолок 2 ч (без +2) ==')
check(MP.ENABLED is False, 'прогрессия выключена')
...
print('== 2. bump не поднимает потолок ==')
MP.bump_after_mute(G, U)
check(MP.step_for(G, U) == 0, 'step остаётся 0')
check(MP.cap_seconds(G, U) == 2 * 3600, 'cap всё ещё 2ч')
MP.bump_after_mute(G, U)
MP.bump_after_mute(G, U)
check(MP.cap_seconds(G, U) == 2 * 3600, 'после 3 bump — всё ещё 2ч')
check(SL.resolve_mute_cap(G, U, []) == 2 * 3600, 'resolve после bump = 2ч')

print('== 3. reset_on_warn — no-op ==')
MP.reset_on_warn(G, U)
check(MP.cap_seconds(G, U) == 2 * 3600, 'после варна потолок тот же')

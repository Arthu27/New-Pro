...
Проверяет структуру 15 инструментов, ролевую фильтрацию, новый светлый
...
print('== 1. Карта инструментов (Модерация 15 + Защита 6) ==')
...
check(len(raw_pages) == 15, f'в разделе модерации ровно 15 инструментов ({len(raw_pages)})')
check(len(prot_group['pages']) == 6,
      f'«Защита» — отдельная категория из 6 страниц ({len(prot_group["pages"])})')
...
check(len({page['path'] for page in _all}) == 21, 'URL всех 21 инструментов уникальны')
...
check(section_counts == [5, 5, 5], f'workflow разбит 5/5/5 — защита отдельной категорией ({section_counts})')
...
check(len(admin_prot['pages']) == 6 and not admin_prot.get('sections'),
      '«Защита» — плоская категория из 6 страниц')
...
admin_only = {'/bulk-actions', '/antiraid', '/guardian'}
check(len(mod_group['pages']) + len(mod_prot['pages']) == 18,
      f'модератор видит 18 доступных инструментов ({len(mod_group["pages"]) + len(mod_prot["pages"])})')

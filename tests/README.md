...
| `test_antifake_analytics_pro.py` | страйки антифейка (файл-первый, гильдии не смешиваются, CSV, сброс), антифейк без бота (статус/тумблеры/порог/защита/лаборатория), Про-аналитика (демо-тикеты, цвета темы), Аналитика (нет var()/color-mix в Chart.js), турецкий текст вычищен, спарклайны с сеткой/подписями/сглаживанием |
| `test_recruit_feed.py` | набор команды (кнопка «Посмотреть», кликабельные карточки, поиск, модалка с инфо о решении) и лента «Последние действия» мод-центра (полноширинная, иконки действий, причины, топ-10) |
| `test_visibility_roles_pro.py` | видимость уведомлений/ленты по ролям (конфиг panel_visibility, гарды API, скрытие кнопок), имена вместо ID в логах, цветные роли (сохранение/валидация/демо-публикация), роли по реакциям (демо-создание, уникальные ID), профиль BOT_SLIM |

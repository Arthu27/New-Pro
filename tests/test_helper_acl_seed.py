...
print('== 2. Лимиты хелпера → меню mute/clear ==')
...
check(lm.get('mute') == 3 and lm.get('unmute') == 3 and lm.get('clear') == 10,
      f'хелпер limits mute/unmute/clear={lm.get("mute")}/{lm.get("unmute")}/{lm.get("clear")}')

print('== 3. /modpanel меню ==')
g = _Guild(GID)
h = _Member(10, [GID, HELPER])
m = _Member(11, [GID, MOD])
# модеру нужны его лимиты без узкого scoped — не ставим role limits на MOD
h_acts = [a[0] for a in actions_for_member(g, h)]
check(set(h_acts) <= {'mute', 'unmute', 'clear'},
      f'хелпер только чат: {h_acts}')
check('ban' not in h_acts and 'warn' not in h_acts,
      f'хелпер без ban/warn: {h_acts}')
...
os.remove('data/.helper_acl.v3') if os.path.exists('data/.helper_acl.v3') else None
for p in ('data/action_acl.json', 'data/.role_seed.v5'):
    if os.path.exists(p):
        os.remove(p)
# permission_acl хранит в sqlite — сбросим через save пустого
pacl.save_action_acl(GID, {})
SEED = {
    'version': 5,
    'role_map': {HELPER: 'mod', MOD: 'mod'},
    'action_default': {'tiers': ['mod']},
}
...
print(f'\n=== PASS {PASS} / FAIL {FAIL} ===')
print('\n--- Хелпер (чат) ---')
print(f'  права:  {", ".join(HELPER_ACTIONS)} (+ размут чата)')
print(f'  лимиты: mute/unmute {HELPER_LIMITS["mute"]}, clear {HELPER_LIMITS["clear"]}')
print('--- Модератор ---')
print('  права:  полный ACL (ban/warn/mute/vmute/timeout/purge/…)')

...
for rid, label, want in ((MOD_ROLE, 'Модер', 3), (CUR_ROLE, 'Куратор', 2),
                         (ADM_ROLE, 'Админ', 2)):
...
print('== 2. Наберём расходку: 2 варна + 10 чисток (куратор) ==')
...
check(not ok and 'Лимит исчерпан' in deny,
      'третий варн куратора в тот же день уже запрещён', f'→ {deny}')
for i in range(10):
...
import json
out = {}
# варн: метки прошлого процесса живы — 3-й варн куратора запрещён
...
check(out.get('warn_denied') is True,
      'после рестарта 3-й варн куратора по-прежнему запрещён (метки живы)',
      f'→ {out}')
...
check(out.get('warn_limit') == 2 and out.get('clear_limit') == 10,
      'лимиты warn=2/clear=10 (куратор) в новом процессе те же (конфиг с диска)')

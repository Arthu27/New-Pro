...
check(_lm_mod['warn'] == 3 and _lm_cur['warn'] == 2 and _lm_adm['warn'] == 2,
      'варны по тирам: модер 3(дефолт, но меню скрыто) / куратор 2 / админ 2')
...
# Потолок длительности: прогрессия (первый шаг 2 ч); effective_max — тир.
check(SL.effective_max_duration(GT, 'mute', [1001]) == 2 * 3600,
      'модер: тировый потолок 2 часа (первый шаг)')
check(SL.effective_max_duration(GT, 'mute', [1002]) == 2 * 3600,
      'куратор: тировый потолок 2 часа')
check(SL.effective_max_duration(GT, 'mute', [1003]) == 2 * 3600,
      'админ: тировый потолок 2 часа')
...
SL.set_role_durations(GT, 1001, who='t', role_name='Мод', mute=4 * 3600)
check(SL.effective_max_duration(GT, 'mute', [1001]) == 4 * 3600,
      'свой потолок роли (4 ч) перебивает тировые 2 часа')
...
check(SL.resolve_mute_cap(GT, 555, [1001]) == 2 * 3600, 'resolve: первый мут 2ч')
MP.bump_after_mute(GT, 555)
check(SL.resolve_mute_cap(GT, 555, [1001]) == 4 * 3600, 'resolve: после мута 4ч')
MP.reset_on_warn(GT, 555)
check(SL.resolve_mute_cap(GT, 555, [1001]) == 2 * 3600, 'resolve: после варна снова 2ч')

...
check(by_key0['warn_steps']['link'] == '/warn-config'
      and by_key0['warn_reasons']['link'] == '/mod-studio', 'ссылки ведут на настройку')
...
DS = '/api/guild/777/mod-studio/dossier'
...
login('uye')
check(client.get('/mod-studio').status_code == 403, 'uye нельзя на страницу')
check(client.get(OV).status_code == 403, 'uye нельзя в снимок')
login('mod')
page = client.get('/mod-studio')
check(page.status_code == 200 and 'Студия модерации' in page.get_data(as_text=True),
...
for fid in ('sdTrend', 'sdTeam', 'sdEffect', 'sdSubjects',
            'sdSubjectsSearch', 'sdDossier', 'sdDaysTabs', 'sdFeed'):
    check(('id="' + fid + '"') in tpl, f'блок {fid} на месте')
check('/mod-studio' in tpl, 'API-путь Центра в шаблоне')
check('/member-card?user=' in tpl, 'из досье есть мост в карточку участника')
check('sdUseInControl' in tpl, 'из досье есть мост в быстрые действия')
check('sdCopy' in tpl, 'копирование ID из досье')

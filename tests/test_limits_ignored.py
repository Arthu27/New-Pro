...
1) «Модер дал 100000 минут мута» — потолок действует ВСЕГДА:
   фиксированный потолок 2 ч (прогрессия «+2» отменена 2026-09-25).
   Минимум 30 мин.
...
print('== 1. Потолок длительности: фиксированный 2 часа ==')
...
check(SL.resolve_mute_cap(GID, 3000000000000000300, []) == 2 * 3600,
      'фиксированный потолок 2ч (без +2)')
...
async def cap_test():
    ...
    ok, text = await cog.apply_panel_action(
        ...
        actor='Модер', duration_cap=None)
    check(not ok and 'дольше разрешённого' in text,
          '3 часа при потолке 2ч — отказ', f'→ {text[:80]}')
    ok, text = await cog.apply_panel_action(
        guild, target, 'timeout', reason='тест', amount='2ч',
        actor='Модер', duration_cap=None)
    check(ok, '2 часа — потолок, выдан', f'→ {text[:80]}')
    # +2 отключён: повторный мут на 2ч всё ещё ок, на 3ч — нет
    ok, text = await cog.apply_panel_action(
        guild, target, 'timeout', reason='тест', amount='2ч',
        actor='Модер', duration_cap=None)
    check(ok, '2 часа повторно (без прогрессии) — можно', f'→ {text[:80]}')
    ok, text = await cog.apply_panel_action(
        guild, target, 'timeout', reason='тест', amount='3ч',
        actor='Модер', duration_cap=None)
    check(not ok and 'дольше' in text,
          '3 часа после мутов всё ещё отказ (нет +2)', f'→ {text[:80]}')
    ok, text = await cog.apply_panel_action(

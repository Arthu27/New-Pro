...
async function loadRolesAndMembers() {
  await Promise.all([loadRoles(\'role-sel\'), loadMembers(\'member-sel\')]);
}
...
function openCmd(cmd) {
  ...
  var forms = {
    ...
    role: \'<div class="form-group"><label>Сервер</label><select id="guild-sel" onchange="loadRolesAndMembers()">\'+guildOptions()+\'</select></div><div class="form-group"><label>Участник</label><select id="member-sel"><option>Загрузитьniyor...</option></select></div><div class="form-group"><label>Роль</label><select id="role-sel"><option>Загрузитьniyor...</option></select></div><button class="btn-exec" onclick="execCmd()">UYGULA</button><button class="btn-cancel" onclick="closeCmd()">Отмена</button>\'
  };
  ...
  setМут(function() {
    ...
    if (cmd === \'role\') loadRolesAndMembers();
  }, 50);
}
...
with open('discord_бот/web/templates/execute_command.html', 'w', encoding='utf-8') as f:
    f.write(content)

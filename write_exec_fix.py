...
async function loимяMembers(selId) {
 var gid = document.getElementById(\'guild-sel\').value;
 if (!gid) return;
...
async function loимяРольesAndMembers() {
 await Promise.all([loимяРольes(\'роли-sel\'), loимяMembers(\'member-sel\')]);
}
...
function openCmd(cmd) {
 ...
 if (cmd === \'роли\') loимяРольesAndMembers();
 }, 50);
}
